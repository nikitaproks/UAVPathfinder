...
def haversine_distance(coord_1: List[float], coord_2: List[float]) -> float:
    # NOTE: this function may no longer be needed, but who knows...
    """
    ...
    try:
        ...
        raise ValueError("Error calculating Haversine distance.") from e


def coord_to_cart(relative_coord: List[float], coord: List[float]) -> List[float]:
    """
    ...
    try:
        ...
        raise ValueError("Error converting coordinates to Cartesian.") from e
